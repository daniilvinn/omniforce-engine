...
namespace Omni {
	...
	void PhysicsEngine::LaunchRuntime(Scene* context)
	{
		...
		for (auto& e : rb2d_view) {
			...
			body_creation_settings.mRotation = JPH::Quat(trs.rotation.x, trs.rotation.y, trs.rotation.z, 1.0f);
			body_creation_settings.mUserData = entity.GetComponent<UUIDComponent>().id.Get();

			const JPH::BodyID& body_id = body_interface.CreateAndAddBody(body_creation_settings, JPH::EActivation::Activate);
			...
			if (entity.HasComponent<BoxColliderComponent>()) {
				...
				body_interface.SetShape(body_id, box_shape_settings.Create().Get(), true, JPH::EActivation::Activate);
			}
				...
				body_interface.SetShape(body_id, sphere_shape_settings.Create().Get(), true, JPH::EActivation::Activate);
			}
	...
	void PhysicsEngine::Update(float32 step)
	{
		...
		m_CoreSystem->Update(step, (int32)optimal_collision_steps, s_InternalData.temp_allocator, s_InternalData.job_system);
	}

	void PhysicsEngine::FetchResults()
	{
		OMNIFORCE_ASSERT_TAGGED(m_Context, 
			"Cannot fetch results before \"PhysicsEngine::LaunchRuntime()\" is called");
	}
	...
	PhysicsEngine::PhysicsEngine()
	{
		...
		s_InternalData.temp_allocator = new JPH::TempAllocatorImpl(10 * 2048 * 1024);
