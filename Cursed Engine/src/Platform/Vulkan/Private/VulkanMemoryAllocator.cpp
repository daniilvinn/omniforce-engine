...
namespace Cursed {
	...
	VulkanMemoryAllocator::~VulkanMemoryAllocator()
	{
		...
		CURSED_CORE_TRACE("Destroying vulkan memory allocator: ");
		CURSED_CORE_TRACE("\tTotal memory allocated: {0}", m_Statistics.allocated);
		CURSED_CORE_TRACE("\tTotal memory freed: {0}", m_Statistics.freed);
		CURSED_CORE_TRACE("\tIn use at the moment: {0}", m_Statistics.currently_allocated);
	}
	...
	VmaAllocation VulkanMemoryAllocator::AllocateBuffer(VkBufferCreateInfo* create_info, uint32_t flags, VkBuffer* buffer)
	{
		...
		if (CURSED_TRACE_DEVICE_ALLOCATIONS) {
			CURSED_CORE_TRACE("Allocating device buffer:");
			CURSED_CORE_TRACE("\tRequested size: {0}", create_info->size);
			CURSED_CORE_TRACE("\tAllocated size: {0}", allocation_info.size);
			CURSED_CORE_TRACE("\tCurrently allocated: {0}", m_Statistics.currently_allocated);
		}
		return allocation;
	}
	...
	void VulkanMemoryAllocator::DestroyBuffer(VkBuffer* buffer, VmaAllocation* allocation)
	{
		...
		if (CURSED_TRACE_DEVICE_ALLOCATIONS) {
			CURSED_CORE_TRACE("Destroying device buffer:");
			CURSED_CORE_TRACE("\tFreed memory: {0}", allocation_info.size);
			CURSED_CORE_TRACE("\tCurrently allocated: {0}", m_Statistics.currently_allocated);
		}
